...
export default function ChatPage() {
 ...
 const API_BASE = "http://127.0.0.1:8001";
  ...
  const deleteSession = async (sessionIdToDelete) => {
    ...
    try {
      // optimistically remove from recentChats
      setRecentChats((prev) => {
        const next = prev.filter((c) => c.id !== sessionIdToDelete);
        persistRecentChats(next);
        return next;
      });

      // If deleting current session, start a fresh chat
      ...
      <div className="sidebar-section">
        ...
        {recentChats.length === 0 ? (
          ...
          recentChats.map((chat) => (
            <div key={chat.id} style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <button
                type="button"
              ...
              {/* Delete button for each recent chat */}
              <button
                onClick={(e) => {
                  e.stopPropagation();
